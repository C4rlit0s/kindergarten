...
if( !function_exists('_the_core_action_theme_includes_tgm') ) :
	...
	function _the_core_action_theme_includes_tgm() {
		$theme_id = defined('FW') ? fw()->theme->manifest->get_id() : 'kids-play';
	...
	function _the_core_action_shortcode_tab_slider_enqueue_dynamic_css($data) {
		...
		foreach ( $atts['tabs'] as $tab_key => $tab ) {
			...
			if ( !empty($tab_content_text) ) {
				$final_styles .= '.fw-tabs-slider .fw-tabs-slider-content-wrap .tf-sh-'.esc_attr( $atts['unique_id'] ).'-'.$tab_key.' .fw-tabs-slider-content .fw-tabs-slider-description {' . $tab_content_text . '}';
				// responsive tab slider title styling
...
if (!function_exists('_the_core_filter_show_404_page')):
	/**
	 * Show custom 404 page
	 */
	function _the_core_filter_show_404_page()
	{
	...
	add_filter('404_template', '_the_core_filter_show_404_page');
endif;
...
function _the_core_filter_fw_ext_backups_demos($demos)
{
	$demos_array = array(
		'kids-play' => array(
			'title' => esc_html__('Kids Play', 'the-core'),
			'screenshot' => 'http://updates.themefuse.com/demos/screenshots/kids-play.png',
			'preview_link' => 'http://demo.themefuse.com/kids-play/',
		)
	);
...
if (!function_exists('_the_core_filter_update_footer')):
	...
	function _the_core_filter_update_footer($html)
	{
		...
		$theme_id = ( defined('FW') && function_exists('fw') ) ? fw()->theme->manifest->get_id() : 'kids-play';
...
if ( !function_exists('_the_core_action_include_predefined_templates') ) :
	...
	function _the_core_action_include_predefined_templates() {
		require_once dirname(__FILE__) .'/includes/predefined-templates/init.php';
	}
endif;
