...
if ( ! function_exists( 'the_core_image' ) ) {
	...
	function the_core_image( $attachment = null, $args = null ) {
		...
		if ( $isbg ) {
			...
			if ( $enable_responsive_images ) {
				$img = '<noscript itemscope itemtype="http://schema.org/ImageObject" itemprop="image"><img src="' . $file_url . '" ' . $html_attr . ' />';
				// meta for markup
				$img .= '<meta itemprop="url" content="'.$file_url.'">';
				if( isset($imagedata['width']) ) $img .= '<meta itemprop="width" content="'.$imagedata['width'].'">';
				if( isset($imagedata['height']) ) $img .= '<meta itemprop="height" content="'.$imagedata['height'].'">';
				// meta for markup
				$img .= '</noscript>';
...
if ( ! function_exists( 'the_core_paging_navigation' ) ) :
	...
	function the_core_paging_navigation( $wp_query = null ) {
		...
		if ( $links ) : ?>
			<nav class="navigation paging-navigation <?php echo $the_core_pagination_type; ?>" role="navigation">
					...
					<?php if ( $the_core_logo_settings['logo']['selected_value'] == 'image' ) :
						...
						if ( ! empty( $image_logo ) ) : ?>
							<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="fw-site-logo">
								<img src="<?php echo esc_url($image_logo['url']); ?>" alt="<?php echo esc_url( bloginfo('name') ) ?>" />
							</a>
						<?php endif;
					else :
						if ( $the_core_logo_settings['logo']['text']['title'] != '' ) : ?>
							<a href="<?php echo esc_url( home_url('/') ); ?>" class="fw-site-logo">
								<strong class="site-title" itemprop="headline"><?php echo $the_core_logo_settings['logo']['text']['title']; ?></strong>
								<?php if ( $the_core_logo_settings['logo']['text']['subtitle'] != '' ) : ?>
									<span class="site-description" itemprop="description"><?php echo $the_core_logo_settings['logo']['text']['subtitle']; ?></span>
								<?php endif; ?>
							</a>
						<?php endif;
					endif; ?>
		...
		if($post_date != 'no' || $post_author != 'no' || $post_categories != 'no') : ?>
			<div class="wrap-entry-meta">
				...
				<?php if ( $post_date != 'no' ) : ?>
					<span class="entry-date">
						<a rel="bookmark" href="<?php echo esc_url($the_core_permalink); ?>">
							...
							<meta itemprop="dateModified" content="<?php echo the_modified_date( 'F j, Y' ); ?>">
						</a>
					</span>
				<?php endif; ?>
				<?php if ( $post_author != 'no' ) : ?>
					...
					<?php endif; ?>
					<span itemscope="itemscope" itemtype="http://schema.org/Person" itemprop="author" class="author"> <?php esc_html_e( 'By', 'the-core' ); ?> <a rel="author" href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>"><span itemprop="name"><?php the_author(); ?></span></a></span>
...
if( ! function_exists( 'the_core_header_image' ) ) :
	...
	function the_core_header_image(){
		...
		if(!empty($image) || (isset($posts_header_overlay_options['posts_header_overlay']) && $posts_header_overlay_options['posts_header_overlay'] =='yes') || (isset($posts_header_overlay_options['category_header_overlay']) && $posts_header_overlay_options['category_header_overlay'] =='yes') ){ ?>
			<section class="fw-main-row-custom <?php echo esc_attr($header_image_overlap_class); ?> fw-main-row-top fw-content-vertical-align-middle <?php echo esc_attr($posts_header_height); ?> fw-section-image fw-section-default-page <?php echo esc_attr($post_type); ?>" style="background-image: url('<?php if( !empty( $image ) ) echo esc_url( $image['url'] ); ?>'); <?php echo $header_image_overlap_style; ?>">
				<?php echo $the_core_overlay_style; ?>
				<div class="fw-container">
					<div class="fw-row">
						<div class="fw-col-sm-12" <?php echo $content_position_style; ?>>
							<div class="fw-heading <?php echo esc_attr($heading_alignment); ?>">
								<h1 class="fw-special-title"><?php echo the_core_translate($title); ?></h1>
								<?php if( $description != '' ) : ?>
									<div class="fw-special-subtitle"><?php echo the_core_translate($description); ?></div>
								<?php endif; ?>
							</div>
						</div>
					</div>
...
if( !function_exists('the_core_header_mobile_menu') ) :
	 ...
	 * Header mobile menu
	 */
	function the_core_header_mobile_menu() {
		$the_core_header_settings = defined( 'FW' ) ? fw_get_db_settings_option( 'header_settings' ) : array();
		if( isset($the_core_header_settings['header_type_picker']['header_type']) && ( $the_core_header_settings['header_type_picker']['header_type'] == 'header-5' || $the_core_header_settings['header_type_picker']['header_type'] == 'header-6' ) ) {
			$bg_video_data_attr = '';
			if( $the_core_header_settings['header_type_picker']['header_type'] == 'header-5' ) {
				...
				// enqueue scripts and styles for header type 5
			...
			?>
			<nav id="header-menu" <?php echo $bg_video_data_attr; ?>></nav>
		<?php
		}
	}
endif;
...
if( !function_exists('the_core_style_file_name') ) :
	...
	function the_core_style_file_name() {
		return apply_filters( '_filter_the_core_style_file_name', 'kids-play-style' );
	}
endif;
